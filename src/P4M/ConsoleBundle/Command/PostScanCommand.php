...
//            ->addOption('yell', null, InputOption::VALUE_NONE, 'Si définie, la tâche criera en majuscules')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('scan '.count($posts).' posts');
...
//            echo $post->getSourceUrl();
            $output->writeln('key'.$postAnalyser->getResponseHTTPCode());
            
            if($postAnalyser->getResponseHTTPCode() !== 200)// 404
            {
                 $post->setQuarantaine(true);
                 $output->writeln('putting to quarantaine '.$postAnalyser->getResponseHTTPCode());
            }
            else
            {
...
//                    die($key);
                    ...
                    if (null !== $author)
                    {
			$output->writeln('got key of that user '.$author->getUsername());
